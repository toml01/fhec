...
pub static CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry { code: "FHE1015", severity: "error", name: "shared-boundary-bad-position", rule: "§2.8", summary: "A shared-boundary marker in an illegal position or shape: `in shared` outside a public/external non-view function, mixed with external `in` inputs, a shared return that is named, part of a tuple, has a recipient other than `msg.sender`, or a `return` that is bare, braceless, or assigns." },
