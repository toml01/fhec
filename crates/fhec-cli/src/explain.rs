...
pub static CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry { code: "FHE1015", severity: "error", name: "shared-boundary-bad-position", rule: "§2.8", summary: "A shared-boundary marker in an illegal position or shape: `in shared` outside an external non-view function, in a `try`/`catch` declaration list, mixed with external `in` inputs, a shared return that is named, part of a tuple, has a recipient other than `msg.sender`, or a `return` that is bare, braceless, assigns, or leaves a rewrite site in a statement an R2 grant already claims." },
    ...
    CatalogEntry { code: "FHE2007", severity: "error", name: "possibly-uninitialized-encrypted", rule: "§6", summary: "A possibly-uninitialized encrypted variable reaches an FHE operation; FHE.select silently substitutes defaults, so this is always an error." },
    ...
    CatalogEntry { code: "FHE2012", severity: "error", name: "shared-boundary-type-mismatch", rule: "§2.8", summary: "The expression a shared return returns is not the encrypted type the function declares it shares." },
