...
pub static CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry { code: "FHE4009", severity: "warning", name: "acl-empty-reader-set", rule: "§8.12", summary: "An encrypted value built entirely from literals through profile operations reaches a public/external return or an emit argument, with no read of any kind; transient grants do not count. Narrowly scoped to what is provably decidable — never a parameter, a state variable, a local, or an internal function's return." },
    ...
    CatalogEntry { code: "FHE4014", severity: "warning", name: "acl-grant-on-unpermissioned-handle", rule: "§8.1", summary: "A storage write or emit argument is a bare `eT.wrap(x)` UDVT cast: CoFHE never registers a permission for it, for anyone, so R1/R4/R5's grant sequence is withheld entirely rather than inserted — it would revert the transaction, not merely fail to help." },
    CatalogEntry { code: "FHE4015", severity: "warning", name: "acl-event-policy-indeterminate", rule: "§8.10", summary: "An emit with an encrypted argument names an event whose declaration is not visible — the emitting contract inherits a base or import the transpiler cannot resolve. A reader policy on the true declaration cannot be seen, so R5 inserts no grants; write them explicitly before the emit, or make every base resolvable." },
