...
pub static CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry { code: "FHE1015", severity: "error", name: "shared-boundary-bad-position", rule: "§2.8", summary: "A shared-boundary marker in an illegal position or shape: `in shared` outside an external non-view function, mixed with external `in` inputs, a shared return that is named, part of a tuple, has a recipient other than `msg.sender`, or a `return` that is bare, braceless, assigns, or leaves a rewrite site in a statement an R2 grant already claims." },
