...
An insertion is suppressed when an equivalent call is already present. **⚠ Draft decision (dedupe window):** "already present" means: a statement calling the same ACL function with an argument that is syntactically identical (after trivial parenthesis stripping) to the would-be inserted argument. Library-syntax calls count only when their base resolves to the trusted profile FHE library; a same-named method on any other base is not a grant. For R1 the window looks **forward**: in the same block after the triggering statement and before the next write to the same location, the next external call, or the end of the block, whichever comes first. For R2 and R3 the window looks **backward**: in the same block before the triggering call or `return`, after the previous write to the granted value, since the grant must precede the statement it serves. Method-syntax calls (`x.allowThis()`) count as equivalent to library-syntax calls (`FHE.allowThis(x)`), subject to the same trust requirement: a method-syntax call counts only when it does not resolve, through an in-unit `using` directive, to a candidate function outside the trusted profile library. CoFHE's own method syntax attaches through `using BindingsEuintN for euintN global;` declared inside `FHE.sol` itself, so an ordinary compilation unit (the profile imported, not vendored in-unit) has no in-unit candidate for these names at all — that absence is trusted by default, and only a competing in-unit binding (e.g. a synthetic `using FakeAcl for euint32;`) is refused. An existing grant modulo the `address(...)` wrapper counts as equivalent for R2.
...
**Readers at an emit site.** A policy's readers resolve under §8.8 unchanged: a parameter of the same event (rule 4) and a state variable (rule 5) are both valid on an event target. Each renders exactly as at an R4 write — one `FHE.allow(<argument>, <reader>)` per reader, in policy order, under the §8.9 zero-address guard unless the reader is provably non-zero — and a bare state-variable reader is re-confirmed unshadowed at the emitting function's scope exactly as §8.9 requires at a write site (FHE4006 when a local or parameter shadows it there). Rule 1's binders do not exist at an emit: an event target has no keys, and `self` binds the location a storage write addresses, so `self` does not resolve on an event target (FHE4005).
...
**Indeterminate declaration — FHE4015, warning.** Name resolution can also fail to produce *any* declaration: when the emitting contract inherits a base the transpiler cannot see (an external or unresolvable import), lookup past the contract's known members degrades to an incomplete-inheritance fallback rather than an event, and the declaration that would carry the policy — the "which declaration" answer above — is outside the analysis. The transpiler MUST NOT read a policy through that fallback: the fallback is what file scope *would* have said, not a resolution, and R5's insertion is a positive action, unlike the read-only classification decisions that may consult it. It also MUST NOT stay silent when it matters: when at least one argument of such an `emit` is independently typed encrypted, the transpiler MUST warn with FHE4015, stating that a policy on the true declaration, if any, is not transcribed — the author must write the grants explicitly before the `emit`, or make every base resolvable. When no argument is encrypted the `emit` produces no diagnostic, on the same conservative under-grant principle §8.2 applies to an `Unknown` callee.
...
| FHE4015 | warning | acl-event-policy-indeterminate (§8.10) — an `emit` with an encrypted argument names an event whose declaration is not visible (unresolvable base or import); a policy on the true declaration, if any, is not transcribed |
