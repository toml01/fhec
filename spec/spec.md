...
R4's `allowThis` is unconditional in the same sense R1's is, and no more: when the written value is `.wrap`-derived (§8.1's FHE4014), the transpiler MUST withhold the entire grant sequence — `allowThis` and every reader — and warn, rather than insert a call that would revert.
...
Like R4, R5 withholds its entire grant sequence and warns with FHE4014 (§8.1) instead when the argument is `.wrap`-derived.
